...
cartRouter.get('/',async (req,res)=>{
    ...
    try{
        if(decoded){
            const notes=await NoteModel.findOne({"userID":decoded.userID}).populate("Fashions.data").populate("electronics.data").populate("appliances.data")
...
cartRouter.post("/Fashion/:id",auth, async (req, res) => {
    ...
    try {
      let cart = await NoteModel.findOne({ userID: decoded.userID })
  
  
      const Fashion = await FashionModel.findById(FashionID)
  
      if (!cart) {
        cart = new NoteModel({ userID: decoded.userID })
      }
...
cartRouter.post("/Appliances/:id",auth, async (req, res) => {
  ...
  cartRouter.patch("/inc/:itemId", async (req, res) => {
    try {
      ...
      const cart = await NoteModel.findOne({ userID: decoded.userID })
      // console.log(cart);
      if (!cart) {
