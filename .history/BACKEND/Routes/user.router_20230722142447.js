    ...
    try {
        ...
        if (user) {
            bcrypt.compare(password, user.password, (err, result) => {
                if (result) {
                    res.status(200).send({ "msg": "login succesfully","name":user.name, "token": jwt.sign({ "userID":user._id }, "jammi", { expiresIn: '3h' }) })
                } else {
...
function token_Generator(res, name, id,image) {
    ...
    res.redirect(`http://127.0.0.1:5500/PROJECT_Front/index01.html?token=${token}&username=${name}&image=${image}`)
